...
public:
    ...
    Teacher(string Name, vector<string>group, vector<string>lessons) 
    {
        this->Name = Name;
        this->group = group;
        this->lesson = lessons;
    ...
    bool validate_teacher()
    {
        if (Name.empty())
        {
            cout << "Ошибка: Имя преподавателя не может быть пустым!" << endl;
            return false;
        }
        if (group.empty())
        {
            cout << "Ошибка: У преподавателя должна быть хотя бы 1 группа!" << endl;
            return false;
        }
        if (lesson.empty())
        {
            cout << "Ошибка: У преподавателя должен быть хотя бы 1 предмет!" << endl;
            return false;
        }
        return true;
    }
...
public:
    ...
    void print_attendance(int num_present, int num_total) {
        ...
        if (num_present >= 0 && num_present <= num_total) {
            cout << "Количество присутсвтующих студентов: " << num_present << endl;
        }
...
int main() {
    ...
    cout << "Дисциплины: ";
    for (string lesson : teacher1.lesson) {
        cout << lesson << " ";
    }
    cout << endl;
    cout << "Группы: ";
    for (string group : teacher1.group) {
        cout << group << " ";
    }   
    cout << endl;
